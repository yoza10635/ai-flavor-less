...
> - **②标准化方差：已改造** —— L3 判据由原始 σ 换为章级 CV=σ/mean（`cv_thresh`，高→关注）；σ 仅展示
> - **③信息密度：已退役（2026-09-15）** —— 40 对盲判复核精确率 7.5%，句对级不存在车轱辘话签名（宪章 §3.4）；
>   `--density` 留作实验遗址，不进报告/判据；现象降级为开放项
> - **合取判据改 2/2 制**（①+②，宪章 §3.2）；工具侧输出格式（超标计数）未实现
